...
Vex.Flow.Beam.prototype.init = function(notes) {
  ...
  // Stem direction should be the direction of the furthest note from the center
  // TODO: this doesn't handle chords yet.
  var line = 0;
  var dir = notes[0].getStemDirection();
  for (var i = 0; i < notes.length; i++) {
    if (Math.abs(notes[i].keyProps[0].line - 3) > line) {
      line = Math.abs(notes[i].keyProps[0].line - 3);
      dir = notes[i].getStemDirection();
    }
  }
  
  // All notes in a beam should have the same stem direction.
  for (var i = 0; i < notes.length; i++) {
    notes[i].stem_direction = dir;
  }
  
  // Validate beam line, direction and ticks.
  this.stem_direction = dir;
